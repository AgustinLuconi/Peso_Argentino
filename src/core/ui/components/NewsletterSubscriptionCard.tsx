        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1.5">
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-mono font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full bg-gold/15 text-gold border border-gold/30 flex items-center gap-1">
                <Sparkles size={11} /> BRIEFING FINANCIERO · PESO ARGENTINO
              </span>
            ...
            <p className="text-xs sm:text-sm text-slate-300 max-w-2xl leading-relaxed">
              Recibí directamente en tu correo el análisis de cotizaciones del dólar, brecha cambiaria,
              curva de bonos, balance del BCRA y mercados internacionales.
            </p>
          </div>

          {/* Social Proof & Sample Preview Button */}
          <div className="flex flex-row md:flex-col items-start md:items-end justify-between gap-2 shrink-0">
            <div className="flex items-center gap-1.5 text-xs text-gold font-mono font-bold bg-black/40 px-3 py-1.5 rounded-lg border border-gold/20">
              ...
              <span>+1.420 analistas y tesorerías</span>
            </div>
            <button
              type="button"
              onClick={() => setIsPreviewOpen(true)}
              className="text-xs font-sans text-slate-300 hover:text-white flex items-center gap-1.5 transition-colors underline decoration-gold/50 underline-offset-4"
            >
              <Eye size={13} className="text-gold" /> Ver ejemplar de muestra
            </button>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2.5">
              <div className="flex items-center justify-between">
                <span className="text-xs font-mono font-bold text-gold uppercase tracking-wider flex items-center gap-1.5">
                  <Clock size={13} /> 1. Frecuencia de Envío (Selecciona una):
                </span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                ...
                >
                  ...
                  <span className="text-[11px] text-slate-400 mt-1">Resumen & Agenda semanal</span>
                </button>
                ...
                >
                  ...
                  <span className="text-[11px] text-slate-400 mt-1">Balance integral a fin de mes</span>
                </button>
              </div>

              {/* Opción Adicional: Alertas Flash */}
              <label className="flex items-center gap-2.5 p-2.5 rounded-lg bg-black/30 border border-white/5 cursor-pointer hover:border-gold/30 transition-colors w-full sm:w-auto">
                <input
                  type="checkbox"
                ...
                <span className="text-xs text-slate-300 flex items-center gap-1.5 font-medium">
                  ...
                  <strong>Alertas Flash & Medidas Urgentes</strong> (Publicación de IPC INDEC en vivo y anuncios BCRA/MECON)
                </span>
              </label>
            </div>

            {/* 2. Catálogo de Temas de Interés (Selección Múltiple de Ambos) */}
            <div className="space-y-3 pt-2 border-t border-white/10">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <span className="text-xs font-mono font-bold text-gold uppercase tracking-wider flex items-center gap-1.5">
                  <Sparkles size={13} /> 2. Temas de Interés (Podes elegir locales e internacionales):
                </span>
                <div className="flex items-center gap-2 text-[11px] font-mono">
                  <button
                    type="button"
                    onClick={handleSelectAll}
                    className="text-slate-400 hover:text-gold transition-colors underline"
                  >
                    Seleccionar Todos ({ALL_NEWSLETTER_TOPICS.length})
                  </button>
              ...
              <div className="space-y-1.5">
                <span className="text-[11px] font-mono text-slate-400 flex items-center gap-1">
                  <Building2 size={12} className="text-sky-400" /> 🇦🇷 MERCADO LOCAL & ARGENTINA
                </span>
                  ...
                  {LOCAL_TOPICS.map((topic) => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-sans font-medium transition-all flex items-center gap-1.5 border ${
                          isSelected
                      ...
                      >
                        {topic.badge}
                        {isSelected && <Check size={12} className="text-gold ml-0.5" />}
                      </button>
                    );
                  })}
                </div>
              ...
              <div className="space-y-1.5 pt-1">
                <span className="text-[11px] font-mono text-slate-400 flex items-center gap-1">
                  <Globe size={12} className="text-emerald-400" /> 🌎 MERCADO INTERNACIONAL & GLOBAL
                </span>
                <div className="flex flex-wrap gap-2">
            ...
            <div className="pt-3 border-t border-white/10 space-y-3">
              <div className="flex flex-col sm:flex-row items-center gap-3">
                <div className="relative flex-1 w-full">
                  ...
                  <input
                    ...
                    placeholder="Ingresá tu correo electrónico (ej. analista@empresa.com)"
                    required
                    disabled={loading}
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full sm:w-auto px-6 py-3 bg-gold hover:bg-gold-light text-slate-950 font-sans font-extrabold text-xs sm:text-sm rounded-xl shadow-lg shadow-gold/20 flex items-center justify-center gap-2 transition-all transform active:scale-95 shrink-0 disabled:opacity-50"
                >
                  {loading ? (
              ...
              <div className="flex items-center justify-between text-[11px] text-slate-400 font-sans">
                <span>🔒 Cero spam. Podés desuscribirte o cambiar de temas con un solo clic.</span>
                <span className="font-mono text-slate-500 hidden sm:inline">PERSISTIDO EN NEON POSTGRESQL</span>
              </div>
            </div>
          </form>
        )}
